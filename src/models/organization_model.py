...
from sqlalchemy import String, ForeignKey, Column, Table, Numeric
...
class BuildingDB(Base):
    ...
    geolocation: Mapped[WKBElement] = mapped_column(Geometry(geometry_type="POINT", srid=4326, spatial_index=True))
...
class ActivityDB(Base):
    ...
    organization: Mapped[list["OrganizationDB"]] = relationship(secondary=organization_activity_association,
                                                                back_populates="activity")
    parent_id: Mapped[Optional[int]] = mapped_column(ForeignKey('activities.id'), nullable=True)
    parent: Mapped[Optional["ActivityDB"]] = relationship(remote_side='ActivityDB.id', back_populates='children')
...
class PhoneDB(Base):
    ...
    def validate_number(self, number):
        ...
        if not re.match(pattern, number):
            raise ValueError("Invalid phone number format.  Must be like: 2-222-222, 3-333-333, 8-923-666-13-13")
        return number
