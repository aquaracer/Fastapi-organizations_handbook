...
class OrganizationRepository:
    ...
    async def list_organizations_by_location(
        ...
        buildings_query = select(BuildingDB).where(
            ST_DWithin(BuildingDB.geolocation, target_geography, 1000 * radius),
        )
        async with self.db_session as session:
            buildings: list[BuildingDB] = (
                (await session.execute(buildings_query)).scalars().all()
            )
            ...
            organizations: list[OrganizationDB] = (
                (await session.execute(organization_query)).scalars().all()
            )
    ...
    async def search_organizations_by_activity(
        ...
        async with self.db_session as session:
            parent_activity: ActivityDB = (
                (await session.execute(query)).scalars().first()
            )
