...
from src.organizations_handbook.exceptions.organization_exceptions import (
...
class OrganizationService:
    ...
    async def get_organization(self, organization_id: int) -> OrganizationSchema:
        if organization := await self.organization_repository.get_organization(
    ...
    async def list_organizations_by_building(
        ...
        organizations = (
            await self.organization_repository.list_organizations_by_building(
                building_id=building_id
            )
        )
        return [
            await self._make_organization_schema(organization=organization)
    ...
    async def list_organizations_by_location(
        ...
        buildings, organizations = (
            await self.organization_repository.list_organizations_by_location(
                latitude=latitude, longitude=longitude, radius=radius
            )
        )
        buildings_schema = [
            BuildingSchema.model_validate(building) for building in buildings
        ]
        ...
        return OrganizationBuildingSchema(
            organizations=organizations_schema, buildings=buildings_schema
        )

    async def list_organizations_by_activity(
        self, activity: str
    ) -> list[OrganizationSchema]:
        if organizations := await self.organization_repository.list_organizations_by_activity(
            activity=activity
        ):
            return [
                await self._make_organization_schema(organization=organization)
    ...
    async def search_organizations_by_activity(
        ...
        if organizations := await self.organization_repository.search_organizations_by_activity(
            activity=activity
        ):
            return [
                await self._make_organization_schema(organization=organization)
    ...
    async def _make_organization_schema(
        self, organization: OrganizationDB
    ) -> OrganizationSchema:
        return OrganizationSchema(
            id=organization.id,
            name=organization.name,
            building=(
                f"{organization.building.city}, " f"{organization.building.address}"
